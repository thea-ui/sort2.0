...
export const ROOM_TYPE_META: Record<string, RoomTypeMeta> = {
  CLASSROOM: { label: 'Classroom', badgeClass: 'bg-sky-100 text-sky-800 border-sky-200', icon: Monitor },
  LABORATORY: { label: 'Laboratory', badgeClass: 'bg-cyan-100 text-cyan-800 border-cyan-200', icon: FlaskConical },
  LIBRARY: { label: 'Library', badgeClass: 'bg-violet-100 text-violet-800 border-violet-200', icon: BookOpen },
  ...
  ADMIN: { label: 'Admin', badgeClass: 'bg-indigo-100 text-indigo-800 border-indigo-200', icon: Building2 },
};
...
export function readableTextColor(hex: string | null | undefined): string {
  ...
  return luminance > 0.6 ? '#00271D' : '#FFFFFF';
}
