  ...
  <div className="space-y-3">
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-full text-xs font-bold flex items-center gap-2 transition-all cursor-pointer shadow-xs ${
            activeFilter === scope
              ...
              : 'bg-white border border-[var(--primary)]/10 text-[var(--text-strong)]/70 hover:bg-[var(--primary)]/5'
          }`}
        >
          {icon} {label}
        </button>
    ...
    <div className="bg-white/90 backdrop-blur-md border border-white/80 rounded-2xl p-4 shadow-sm flex flex-col sm:flex-row gap-3 items-center justify-between">
      ...
      <select
        ...
        className="w-full sm:w-48 px-3 py-2.5 text-xs bg-[var(--primary)]/5 border border-transparent rounded-xl outline-none focus:border-[var(--accent)] font-semibold text-[var(--text-strong)] cursor-pointer"
      >
