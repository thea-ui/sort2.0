...
export const AdminChallengesTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-2xl font-black text-[#00271D] flex items-center gap-2">
            <Target size={22} className="text-[#00A77C]" />
            Challenges
          </h3>
          <p className="text-sm text-[#00271D]/50 mt-0.5">Manage gamification challenges for students.</p>
        </div>
        <button onClick={() => setShowCreate(true)} className="px-4 py-2.5 bg-[#00A77C] hover:bg-[#008f6a] text-white text-sm font-bold rounded-xl shadow-sm flex items-center gap-2 cursor-pointer transition-all">
          <Plus size={14} />
          Create Challenge
        </button>
      </div>
      ...
      {challenges.length === 0 ? (
        <div className="rounded-2xl border border-[#00271D]/10 bg-white p-12 text-center">
          <Target size={40} className="mx-auto text-[#00271D]/20 mb-3" />
          <p className="text-[#00271D]/50 font-medium mb-4">No challenges yet</p>
          <button onClick={() => setShowCreate(true)} className="px-5 py-2.5 bg-[#00A77C] hover:bg-[#008f6a] text-white text-sm font-bold rounded-xl cursor-pointer">
        ...
        <div className="space-y-3">
          {challenges.map(c => (
            <div key={c.id} className={`rounded-2xl border bg-white p-5 transition-all ${c.isActive ? 'border-[#00271D]/10' : 'border-[#00271D]/5 opacity-60'}`}>
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h4 className="font-bold text-[#00271D] truncate">{c.title}</h4>
                    ...
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-[#00A77C]/10 text-[#00A77C] border border-[#00A77C]/20">
                      {CHALLENGE_TYPE_LABELS[c.challengeType]}
                    </span>
                  </div>
                  <p className="text-xs text-[#00271D]/50 mb-2 line-clamp-2">{c.description}</p>
                  <div className="flex items-center gap-4 text-xs text-[#00271D]/50">
                    <span>Target: <strong className="text-[#00271D]">{c.challengeType === 'WEIGHT_COLLECTED' ? `${(c.target / 1000).toFixed(1)} kg` : c.target}</strong></span>
                    <span>Reward: <strong className="text-[#00271D]">{c.pointsAwarded} pts</strong></span>
                    ...
                    {c.startDate || c.endDate ? (
                      ...
                      <span className="text-[#00271D]/30">No window</span>
                    )}
                  </div>
                  {c.stats && (
                    <div className="flex items-center gap-3 mt-2 text-[10px] text-[#00271D]/40">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => handleToggle(c)} className="p-2 rounded-xl hover:bg-[#00A77C]/10 transition-colors cursor-pointer" title={c.isActive ? 'Deactivate' : 'Activate'}>
                    {c.isActive ? <ToggleRight size={20} className="text-[#00A77C]" /> : <ToggleLeft size={20} className="text-gray-400" />}
                  </button>
                  <button onClick={() => setEditing(c)} className="p-2 rounded-xl hover:bg-blue-50 transition-colors cursor-pointer" title="Edit">
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-bold text-[#00271D] mb-2">Delete Challenge?</h3>
            <p className="text-sm text-[#00271D]/60 mb-4">"{deleteConfirm.title}" will be permanently deleted. {deleteConfirm.hasProgress && 'Users have progress — consider deactivating instead.'}</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 text-sm font-bold text-[#00271D]/60 rounded-xl hover:bg-gray-100 cursor-pointer">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-xl">
            <div className="flex items-center gap-2 mb-2">
              ...
              <h3 className="text-lg font-bold text-[#00271D]">Deactivate Challenge?</h3>
            </div>
            <p className="text-sm text-[#00271D]/60 mb-4">{deactivateConfirm.stats?.usersInProgress || 0} users are mid-progress. Deactivating hides it from their list but preserves their progress.</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setDeactivateConfirm(null)} className="px-4 py-2 text-sm font-bold text-[#00271D]/60 rounded-xl hover:bg-gray-100 cursor-pointer">Cancel</button>
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl p-6 max-w-lg w-full shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-bold text-[#00271D]">{isEdit ? 'Edit Challenge' : 'Create Challenge'}</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-[#00271D]/50 uppercase tracking-wider">Title</label>
            <input value={title} onChange={e => setTitle(e.target.value)} className="w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 bg-[#F9F3F0]/60 text-sm text-[#00271D] outline-none focus:border-[#00A77C]" />
          </div>

          <div>
            <label className="text-xs font-bold text-[#00271D]/50 uppercase tracking-wider flex items-center gap-1">
              Code
            ...
            <input value={code} onChange={e => { setCode(e.target.value); setCodeManuallyEdited(true); }} disabled={hasProgress}
              className={`w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 text-sm font-mono outline-none focus:border-[#00A77C] ${hasProgress ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-[#F9F3F0]/60 text-[#00271D]'}`} />
          ...
          <div>
            ...
            <select value={challengeType} onChange={e => setChallengeType(e.target.value as ChallengeType)} disabled={hasProgress}
              className={`w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 text-sm outline-none focus:border-[#00A77C] ${hasProgress ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-[#F9F3F0]/60 text-[#00271D]'}`}>
          ...
          <div>
            ...
            <input type="number" min="1" value={target} onChange={e => setTarget(e.target.value)} disabled={hasProgress}
              className={`w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 text-sm outline-none focus:border-[#00A77C] ${hasProgress ? 'bg-gray-100 text-gray-500 cursor-not-allowed' : 'bg-[#F9F3F0]/60 text-[#00271D]'}`} />
            <p className="text-[10px] text-[#00271D]/40 mt-1">{targetHelp}</p>
          </div>
          ...
          <div>
            <label className="text-xs font-bold text-[#00271D]/50 uppercase tracking-wider">Icon Name</label>
            <input value={iconName} onChange={e => setIconName(e.target.value)} className="w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 bg-[#F9F3F0]/60 text-sm text-[#00271D] outline-none focus:border-[#00A77C]" />
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-bold text-[#00271D]/50 uppercase tracking-wider">Start Date</label>
              <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 bg-[#F9F3F0]/60 text-sm text-[#00271D] outline-none focus:border-[#00A77C]" />
            </div>
            <div>
              <label className="text-xs font-bold text-[#00271D]/50 uppercase tracking-wider">End Date</label>
              <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 bg-[#F9F3F0]/60 text-sm text-[#00271D] outline-none focus:border-[#00A77C]" />
            </div>
          </div>

          <div>
            <label className="text-xs font-bold text-[#00271D]/50 uppercase tracking-wider">Description</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={3} className="w-full mt-1 px-3.5 py-2.5 rounded-xl border border-[#00271D]/10 bg-[#F9F3F0]/60 text-sm text-[#00271D] outline-none focus:border-[#00A77C] resize-none" />
          </div>

          <div className="flex gap-3 justify-end pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2.5 text-sm font-bold text-[#00271D]/60 rounded-xl hover:bg-gray-100 cursor-pointer">Cancel</button>
            <button type="submit" disabled={saving} className="px-5 py-2.5 bg-[#00A77C] hover:bg-[#008f6a] text-white text-sm font-bold rounded-xl shadow-sm flex items-center gap-2 cursor-pointer transition-all disabled:opacity-50">
