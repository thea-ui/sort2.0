...
export const AdminAcademicCalendarTab: React.FC = () => {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      {/* Active Term Hero */}
      {activeTerm && (
        <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-[var(--primary)] via-[var(--primary-light)] to-[var(--primary)] p-8 text-white shadow-2xl shadow-[var(--primary)]/30">
          {/* Decorative circles */}
          ...
          return (
            <div
              ...
              className={`rounded-3xl p-6 border-2 transition-all relative group ${
                isCurrent
                  ? 'bg-white border-[var(--accent)] shadow-lg shadow-[var(--accent)]/10'
                  : isPast
                    ? 'bg-gray-50 border-gray-200 opacity-60'
                    : 'bg-white border-gray-200 hover:border-[var(--accent)]/40 hover:shadow-md'
              }`}
            >
              {isCurrent && (
                <span className="absolute -top-3 left-6 px-3 py-0.5 bg-[var(--accent)] text-white text-[10px] font-black rounded-full uppercase tracking-wider">
                  Active
                </span>
              )}

              <h4 className="text-xl font-black text-[var(--text-strong)]">{t.quarterName}</h4>

              <div className="mt-4 space-y-3">
                <div>
                  <p className="text-[10px] font-bold text-[var(--text-strong)]/40 uppercase tracking-wider mb-0.5">Starts</p>
                  <p className="text-lg font-bold text-[var(--text-strong)]">{formatDate(t.startDate)}</p>
                </div>
                <div className="w-full h-px bg-gray-200" />
                <div>
                  <p className="text-[10px] font-bold text-[var(--text-strong)]/40 uppercase tracking-wider mb-0.5">Ends</p>
                  <p className="text-lg font-bold text-[var(--text-strong)]">{formatDate(t.endDate)}</p>
                </div>
              </div>

              <div className="mt-4 flex items-center justify-between">
                <span className={`text-xs font-bold ${isPast ? 'text-gray-400' : isCurrent ? 'text-[var(--accent)]' : 'text-[var(--text-strong)]/40'}`}>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-8 w-full max-w-md shadow-2xl border border-white space-y-5 animate-fade-in">
            <div className="flex justify-between items-center border-b border-gray-100 pb-4">
              <h3 className="text-2xl font-black text-[var(--text-strong)]">Edit {editingTerm.quarterName}</h3>
              <button type="button" onClick={() => setEditingTerm(null)} className="p-2 rounded-xl hover:bg-gray-100 text-[var(--text-strong)]/40 hover:text-[var(--text-strong)]">
                <X size={20} />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveTermDates} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-[var(--text-strong)]/50 uppercase">Start Date</label>
                <input
                  type="text"
                  required
                  value={editingTerm.startDate}
                  onChange={(e) => setEditingTerm({ ...editingTerm, startDate: e.target.value })}
                  className="w-full mt-1 p-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-semibold text-[var(--text-strong)] outline-none focus:border-[var(--accent)]"
                />
              </div>

              <div>
                <label className="text-xs font-bold text-[var(--text-strong)]/50 uppercase">End Date</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" className="px-6 py-2.5 bg-[var(--accent)] text-white font-extrabold rounded-xl shadow-md text-sm hover:bg-[var(--accent-dark)] cursor-pointer">Save Changes</button>
              </div>
            </form>
          </div>
        </div>
