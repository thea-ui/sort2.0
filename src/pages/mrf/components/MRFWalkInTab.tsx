      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          <div className="lg:col-span-2 bg-gray-50 backdrop-blur-md border border-white/80 rounded-3xl shadow-sm p-5 flex flex-col gap-4">
            ...
            {/* Points preview */}
            <div className="bg-white border border-[var(--accent)]/25 rounded-2xl p-3.5 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Coins size={16} className="text-[var(--gold)]" />
                <span className="text-[11px] font-bold text-[var(--text-strong)]/60 uppercase tracking-wider">Points</span>
              </div>
              <span className="text-xl font-heading font-black text-[var(--accent)]">+{previewPoints}</span>
            </div>

            {/* Milestone progress */}
            {selected && selectedProgress && (
              <div className="bg-white border border-[var(--gold)]/25 rounded-2xl p-3.5 space-y-1.5">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-black uppercase tracking-wider text-[var(--gold)] flex items-center gap-1">
                    <Gift size={11} /> Milestone progress
                  </span>
                  <span className="text-[10px] font-bold text-[var(--text-strong)]/60">
                    {(projectedGrams / 1000).toFixed(2)} kg
                  </span>
                </div>
                <div className="h-2 w-full bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[var(--gold)] to-[var(--gold)] transition-all duration-500"
                ...
                {nextReward ? (
                  <p className="text-[10px] text-[var(--text-strong)]/50 font-semibold">
                    ...
                    <span className="font-bold text-[var(--text-strong)]">{nextReward.title}</span>
                  </p>
                ) : (
                  <p className="text-[10px] text-[var(--accent)] font-bold">All milestone tiers unlocked!</p>
                )}
              </div>
            )}
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <textarea
                  ...
                  className="w-full rounded-xl border border-gray-200 bg-white pl-9 pr-3 py-2.5 text-xs text-[var(--text-strong)] outline-none resize-none focus:border-[var(--accent)] focus:ring-2 focus:ring-[var(--accent)]/20"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              className="mt-auto w-full py-3.5 rounded-xl bg-[var(--accent)] hover:bg-[var(--accent-dark)] text-white text-xs font-extrabold shadow-md shadow-[var(--accent)]/25 transition-all flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
            >
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
            {today.map((turnover) => (
              ...
              >
                ...
                <span className="text-xs font-black text-[var(--accent)] shrink-0">+{turnover.pointsAwarded} pts</span>
              </div>
            ))}
          </div>
        )}
